...
const PieChartProfilesProportion = memo(({ dohnutPopsByProfileData, index, isChartsDataCalculating }) => {
  ...
  const renderLabels = (chart) => {
    ...
    ? `<img style="width: 100%; height: 100%; border: 3px solid ${data.datasets[0].backgroundColor[i]}; border-radius: 50%; object-fit: cover;" src=${data.image[i] ? process.env.REACT_APP_BASE_FIREBASE_PHOTOS_URL + data.image[i] : `${process.env.REACT_APP_BASE_FIREBASE_LOGOS_URL}${generalSettingsData[3]}`}?alt=media />`
