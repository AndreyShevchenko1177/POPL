...
function NotificationModal({ closeModal, data, file }) {
  ...
  const sendNotification = () => {
    if (data.sendAs === 2) {
      ...
      if (file) {
        return dispatch(addAttachementAction(file, { ...reqData, users: data.recipients }, closeModal));
      }
      return dispatch(sendEmailAction({ ...reqData, users: data.recipients }, closeModal));
    }
    setIsButtonDisabled(true);
  ...
  const sendNotificationByTime = () => {
    if (data.sendAs === 2) {
      ...
      setIsButtonDisabled(true);
      if (file) {
        return dispatch(addAttachementShedulerAction(file, { ...reqData, users: reqData.recipients, time: Math.round((new Date(selectedDate).getTime() - new Date().getTime()) / 1000) }, closeModal));
      }
      return dispatch(sendShedulerEmailAction({ ...reqData, users: reqData.recipients, time: Math.round((new Date(selectedDate).getTime() - new Date().getTime()) / 1000) }, closeModal));
    }
    setIsButtonDisabled(true);
